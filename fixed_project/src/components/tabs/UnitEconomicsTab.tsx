...
export function UnitEconomicsTab({ metrics: m }: { metrics: FullMetrics }) {
  const rows = [
    { label: 'Выручка на заказ', value: m.effectiveAverageCheck, fmt: fmtCurrency },
    { label: 'Себестоимость товара', value: -safeNumber(m.cogs / Math.max(m.effectiveOrders + m.repeatOrders, 1)), fmt: fmtCurrency },
    { label: 'Переменные расходы', value: -m.effectiveVariableCostPerOrder, fmt: fmtCurrency },
  ...
  const totalOrderCount = Math.max(m.effectiveOrders + m.repeatOrders, 1);
  const fixedPerOrder = m.totalFixedCosts / totalOrderCount;
