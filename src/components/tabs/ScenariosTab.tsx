...
import { calculateScenario, generateProjections } from '../../calculations';
...
export function ScenariosTab() {
  ...
  const data = useMemo(() =>
    scenarios.map((s) => ({
      ...s,
      m: calculateScenario(
        store.params, s.multipliers, store.revenue, store.marketing,
        store.ltv, store.warehouse, store.oneTimeExpenses, store.monthlyExpenses,
  ...
  const chartData = data.map((s) => ({
    name: s.label,
    'Выручка': s.m.effectiveRevenue,
    'Расходы': s.m.cogs + s.m.totalVariableCosts + store.params.marketingBudget * (s.multipliers.marketingBudget ?? 1) + (s.m.hasDetailedExpenses ? s.m.detailedMonthlyCosts + s.m.totalTeamCost : store.params.fixedCosts),
    'Чистая прибыль': s.m.netProfit,
  }));
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="Сравнение сценариев">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
            ...
            <Bar dataKey="Расходы" fill="#f97316" radius={[4, 4, 0, 0]} />
            <Bar dataKey="Чистая прибыль" fill="#10b981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </SectionCard>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400">
                ...
                {data.map((s) => (
                  <th key={s.name} className="text-right py-2 px-3 font-medium">{s.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              <TRow label="Выручка" values={data.map((s) => fmtCurrency(s.m.effectiveRevenue))} />
      ...
      <SectionCard title="Параметры сценариев">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {data.map((s) => (
            <div key={s.name} className={`p-3 rounded-lg border ${
              s.name === 'pessimistic' ? 'border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-900/10' :
              s.name === 'optimistic' ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-900/10' :
              'border-gray-200 dark:border-gray-700'
            }`}>
              <h4 className="font-medium text-sm mb-2 text-gray-800 dark:text-gray-200">{s.label}</h4>
              <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                {Object.entries(s.multipliers).map(([key, val]) => (
                  <div key={key} className="flex justify-between">
                    <span>{keyLabels[key] || key}</span>
                    <span className={`font-medium ${val > 1 ? (key === 'costPerOrder' || key === 'marketingBudget' ? 'text-red-600' : 'text-emerald-600') : val < 1 ? (key === 'costPerOrder' || key === 'marketingBudget' ? 'text-emerald-600' : 'text-red-600') : ''}`}>
...
const keyLabels: Record<string, string> = {
  ...
  costPerOrder: 'Себестоимость',
  marketingBudget: 'Маркетинг',
};

function TRow({ label, values, colors, bold }: { label: string; values: string[]; colors?: string[]; bold?: boolean }) {
  return (
    <tr className="border-b border-gray-100 dark:border-gray-700">
      <td className={`py-2 px-3 text-gray-700 dark:text-gray-300 ${bold ? 'font-bold' : ''}`}>{label}</td>
      {values.map((v, i) => (
        <td key={i} className={`py-2 px-3 text-right ${bold ? 'font-bold' : 'font-medium'} ${colors?.[i] || 'text-gray-900 dark:text-white'}`}>
          {v}
        </td>
      ))}
    </tr>
